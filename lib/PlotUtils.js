L.SVG.include({
    ...
    _updateArc: function(layer) {
        this._setPath(layer, this._arcPointsToPath(layer));
    },
    _arcPointsToPath: function(layer) {
        var p = layer._point,
            r = layer._radius,
            deg = layer._deg,
            clockwise = layer._clockwise;
        var cw = typeof clockwise !== 'undefined' ? clockwise : 1;
        var x = p.x - r + r * Math.cos(deg * Math.PI / 180);
        var y = p.y + (1 === cw ? 1 : -1) * r * Math.sin(deg * Math.PI / 180);
        var bigOrSmall = deg > 180 ? 1 : 0;
        var line = " L" + (p.x - r) + " " + p.y + " L" + p.x + " " + p.y + "Z";
        return "M " + p.x + " " + p.y + " A " + r + " " + r + " 0 " + bigOrSmall + " " + cw + " " + x + " " + y + line;
    },
});
